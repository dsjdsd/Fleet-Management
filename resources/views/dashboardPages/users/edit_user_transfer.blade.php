                ...
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="body">
                            ...
                            <form name="frm" class="user_transfer" method="POST" action="{{route('update_user_transfer')}}">
                                @csrf
                                <div class="row clearfix">
                                    <div class="col-sm-12">
                                        <div class="row">
                                            <div class="form-group role_main_div col-lg-6 col-sm-12">
                                                ...
                                                <select name="user" id="user" class="w-100 show-tick ms select2">
                                                    ...
                                                    @foreach($users as $key=>$val)
                                                        <option value="{{$val->id}}" {{old('user')==$val->id ? 'selected' : ($transferred_user_detail->user_id==$val->id ? 'selected' : '')}}>{{$val->name}}</option>
                                                    @endforeach
                                                </select>
                                                @error('user')
                                            ...
                                            <div class="form-group role_main_div col-lg-6 col-sm-12">
                                                ...
                                                <select name="district" id="district" class="form-control show-tick ms">
                                                    ...
                                                    @foreach($districts as $key=>$val)
                                                        <option value="{{$val->id}}" {{old('district')==$val->id ? 'selected' : ($transferred_user_detail->district_id ? 'selected' : '')}}>{{$val->district}}</option>
                                                    @endforeach
                                                </select>
                                                @error('district')
                                                <label class="error help-block">{{ $message }}</label>
                                        ...
                                        <div class="form-group role_main_div">
                                                ...
                                                <input type="text" class="form-control date_field"
                                                    ...
                                                    value="{{old('transferred_on') ? old('transferred_on') : date('d-M-Y',strtotime($transferred_user_detail->transferred_on))}}" />
                                                @error('transferred_on')
                                                <label class="error help-block">{{ $message }}</label>
                                                @enderror
                                        </div>
                                        <div class="form-group text-right mt-2">
                                            <input type="hidden" name="transfer_id" value="{{Crypt::encryptString($transferred_user_detail->id)}}"/>
