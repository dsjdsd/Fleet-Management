        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="body">
                            <form name="frm" enctype="multipart/form-data" class="make_form" method="POST" action="{{route('create_service_expenses')}}">
                                @csrf
                                <div class="row clearfix">
                                    <div class="col-sm-12">
                                        <input type="hidden" name="service_expense_id" value="{{$service_expense->id}}">
                                    <div class="row">
                                      <div class="form-group role_main_div col-md-6 col-sm-12">
                                        <label>Vehicle Number</label>
                                            ...
                                            <option value="{{$vehicle->id}}" <?php if($service_expense->vehicle_id==$vehicle->id){echo"selected";}?>>{{$vehicle->registration_number}}</option>
                                            @endforeach
                                            </select>
                                            @error('vehicle_id')
                                        ...
                                        <label>Driver</label> 
                                            <select name="driver_id" id="driver_id" class="w-100 show-tick ms select2">
                                                ...
                                                <option value="{{$driver->id}}" <?php if($service_expense->driver_id==$driver->id){echo"selected";}?>>{{$driver->name}}</option>
                                                @endforeach
                                            </select>
                                            @error('driver_id')
                                                <label class="error help-block">{{ $message }}</label>
                                        ...
                                        <input type="text" name="pno_number" readonly class="form-control" placeholder="PNO Number" id="pno_number" value="{{$service_expense->pno_number}}"> 
                                            @error('pno_number')
                                                <label class="error help-block">{{ $message }}</label>
                                            @enderror
                                        </div>
                                        ...
                                        <input type="number" class="form-control" placeholder="Cost" name="cost" id="cost" value="{{$service_expense->cost}}" />
                                            @error('cost')
                                                <label class="error help-block">{{ $message }}</label>
                                            @enderror
                                        </div>
                                        ...
                                        <input type="text" class="form-control date_field" placeholder="Service Date" name="service_date" id="service_date" value="{{$service_expense->service_date}}" />
                                            @error('service_date')
                                                <label class="error help-block">{{ $message }}</label>
                                            @enderror
                                        </div>
...
$(function(){
    $(".make_form").validate({
        ...
        messages: {
            ...
            service_date: {
                required: "Please select a valid service date"
            },
        }
    });
