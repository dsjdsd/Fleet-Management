                        ...
                        <div class="body">
                            <form name="frm" class="make_form" method="POST" action="{{route('create_vehicle_deployement')}}">
                                ...
                                <div class="row clearfix">
                                    <div class="col-sm-12">
                                    <div class="row">
                                    <input type="hidden" name='vehicle_deployement_id' value="{{$vehicle_deployment->id}}">
                                      <div class="form-group role_main_div col-lg-6 col-sm-12">
                                        ...
                                        <select name="vehicle_id" id="vehicle_id" class="w-100 show-tick ms select2">
                                            ...
                                            <option value="{{$vehicle->id}}" <?php if($vehicle->id==$vehicle_deployment->vehicle_id){echo"selected";}?>>{{$vehicle->registration_number}}</option>
                                            @endforeach
                                        </select>
                                        @error('vehicle_id')
                                        ...
                                        <label>Deployed District</label> 
                                            <select name="deployed_district_id" id="deployed_district_id" class=" form-control show-tick ms">
                                                ...
                                                <option value="{{$district->id}}" <?php if($district->id==$vehicle_deployment->deployed_district_id){echo"selected";}?>>{{$district->district}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <input type="text" class="form-control date_field" placeholder="Deployed Date" name="deployed_date" id="deployed_date" value="{{date('d-M-Y',strtotime($vehicle_deployment->deployed_date))}}" />
                                            @error('deployed_date')
                                                <label class="error help-block">{{ $message }}</label>
                                            @enderror
                                        </div>
